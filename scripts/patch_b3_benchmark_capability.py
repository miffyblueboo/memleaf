...
path = Path("benchmarks/p1/run_baseline.py")
...
if text.count(anchor) != 1:
    raise SystemExit(f"BudgetedModel capability anchor mismatch: {text.count(anchor)}")
path.write_text(text.replace(anchor, replacement, 1), encoding="utf-8")

Path("tests/test_b3_benchmark_harness.py").write_text(r'''from __future__ import annotations
