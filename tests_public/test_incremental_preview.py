...
class IncrementalPreviewTests(unittest.TestCase):
    ...
    def test_current_turn_explicit_non_ascii_title_recalls_target_without_previous_turn(self):
        self.target("mem-report", title="验收报告", body="交付验收报告。", scopes=["global"])
        self.capture("update", "验收报告原定截止日期取消，等待重新确定日期。", offset=4)
    ...
    def test_generic_two_character_title_is_not_promoted_by_sentence_containment(self):
        ...
        self.capture("generic", "这个项目今天有一点普通进展。", offset=4)
        payload = json.loads(self.preview(turn_id="generic")["request"]["user"])
