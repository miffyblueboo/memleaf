...
def _provider_version(data):
    match = re.search(rb'(?m)^version:\s*([0-9A-Za-z.+-]+)\s*def suite_assets(files):
    return {name: data for name, data in files.items()
            if name.startswith(('tests_public/', 'examples/'))}
...
def _wheel_metadata_version(files):
    ...
    versions = [line.split(':', 1)[1].strip() for line in matches[0].decode('utf-8').splitlines()
...
def assert_distribution_versions(wheel_files, source_files):
    try:
        ...
        raise ValueError('sdist_project_version_missing') from error
    values = {
    ...
    if not isinstance(project_version, str) or len(set(values.values())) != 1:
