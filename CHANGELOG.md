...
- State the rule generally, with no worked instance. The contract now says ownership is judged by meaning and never by wording, that a scope already listed in `scope_registry` is reused when the evidence means it even if the wording differs, and that a name does not have to appear in the evidence. A deferral is reserved for the case where the evidence is about a project but which project it is cannot be determined; a fact that simply has no project is `global` and is never deferred. An earlier draft stated that clause backwards and would have deferred every standing preference and environment fact. An earlier draft also carried a worked example naming one real project; an instance only ever helps the project it names, so it was removed and the same stability was obtained from the directive itself.
- Verification used the real model. 115 tests pass with no model call. Replaying the reproduced turn now writes the memory in both variants (with and without the assistant's reply). Four sentences were run against a registry holding one project: the same wording, a paraphrase ("我那个记录账单的小项目，部署挪到 Mac mini 上了"), an indirect reference, and an unrelated new project -- the first three all landed on the existing scope and the fourth correctly minted its own, so paraphrase is recognised and distinct projects are not merged. Six sentences carrying no project (a rule, a preference, an environment fact, a bare product mention) still resolved to `global` rather than deferring.
...
- State the rule as an ownership decision rather than a permission. The first wording ("use a project scope only when the evidence establishes that project") was read as being about *topics*, and the model kept choosing `global`. The shipped wording says that a candidate concerning one distinct project the user is working on -- a `type=project` memory, or the work, decisions, state or deadlines belonging to that project -- is scoped to that project as `project:<name>`, named as the evidence names it, and that `global` is for a fact no single project owns: a standing personal preference, a machine-wide or tool-wide rule, or an environment fact. Nothing had to be built for this: new scope names are already accepted unregistered, `project:记账` is a legal key, and the commit path registers newly observed scopes on write.
- Verification for this release used the real model and the real prompt builder. 102 tests pass with no model call, pinning the vocabulary, the directive phrasing and the `see SCOPES` reference from the compact contract. Against the live route, eight sentences were sent with an EMPTY memory library so the model had to CREATE and only the chosen scope was under test: three project-shaped sentences produced `project:记账`, `project:爬虫` and `project:NAS`, three global-shaped sentences produced `global`, a bare product mention ("我看了下 FastAPI 的文档") stayed `global` rather than claiming ownership, and someone else's project produced no memory at all. Two full passes agreed on all eight cases. Whether a longer or noisier turn keeps this discrimination is not measured, and the 23 memories already written as `global` are not re-scoped by this release.
