...
每条记忆保留主体、时间、条件、否定和状态。按主体/目标/上下文识别事项，不凭标题。交接/完成用 UPDATE 同改 title/body，交接另给 assignee；按用户最新范围改写事实，排除部分不留待办。传材料不转责；独立且有证据的责任保留。请求未必已执行，结果需证据。历史留版本，当前不追加日志。核心身份/目标不明才延期。
## JSON 协议
