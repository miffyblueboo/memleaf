...
INCREMENTAL_SYSTEM = """为 Memleaf 整理长期协作记忆，不做对话摘要。输入中 use=new 的 user 与 final assistant 共同组成一个完整本轮；先整体判断本轮是否产生需要长期保留或维护的信息，再生成操作。不要逐消息结算，也不要因助手寒暄单独输出 NO_MEMORY。assistant 可帮助解释用户本轮指代或重述结果，但不能单独建立用户偏好、决定、承诺或责任；这些需用户表达或确认。context 仅用于迟到/恢复安全，不作为独立新事项来源。
