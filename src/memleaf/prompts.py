...
OUTPUT CONTRACT
A normal summary requires title, body, tags, type, scopes, and sources. Optional existing-schema fields are memory_id, update_memory_id, aliases, keywords, scope_source, evidence_event_ids, shadow_native_ids, scope_operations, actionable, status, assignee, waiting_on, completed_at, and due_date. Use only admitted current event keys in sources/evidence references. Copy the Gate candidate's type and scopes exactly; if scope_source is present, it must match the Gate value.

SOURCES SHAPE
""" + _SOURCES_CONTRACT + """

ACTION STATE AND DATES
Evidence events may include an ISO-8601 UTC timestamp. For an independently trackable action of any type set actionable=true and include status and due_date; use due_date=null when no deadline is established. Existing todo is implicitly actionable. A person or date alone is not an action. For an updated action include current status. Delegation is not completion; a completed side effect does not complete its parent project. For completed, include completed_at only when admitted Evidence establishes the actual completion time, or preserve a trusted existing value; a message timestamp alone is not completion time. Do not invent dates. Preserve only date meaning supported by admitted Evidence; an admitted visible-message timestamp may anchor a supported relative date. Core normalizes supported relative calendar dates to YYYY-MM-DD and validates grounding; recurring schedules may remain recurring.
