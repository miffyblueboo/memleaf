...
_COMPACT_FIELDS = frozenset(("body", "source_memory_ids"))
COMPACT_MAX_BYTES = 128 * 1024
COMPACT_MAX_ITEMS = 32
...
def validate_compact_output(
    ...
    """Validate body-only proposals; all business metadata belongs to Core.

    This is the new-request contract. Existing compaction journals contain
    staged Markdown and hashes, not model JSON, and retain their own reader.
    """
    if isinstance(value, str):
        return parse_compact_output(value, current_memory_ids)
    if not isinstance(value, Mapping):
        raise ModelOutputError("compact output must be an object")
    _require_keys(value, {"memories"}, {"memories"})
    memories = value["memories"]
    if not isinstance(memories, list) or len(memories) > COMPACT_MAX_ITEMS:
        raise ModelOutputError("compact memories must be a bounded list")
    allowed_ids = set(current_memory_ids or ())
    consumed: set[str] = set()
    normalized: list[dict[str, Any]] = []
    for memory in memories:
        if not isinstance(memory, Mapping):
            raise ModelOutputError("each compact replacement must be an object")
        _require_keys(memory, set(_COMPACT_FIELDS), set(_COMPACT_FIELDS))
        body = memory["body"]
        _string(body, "compact body", multiline=True)
        if len(body.encode("utf-8")) > COMPACT_MAX_BYTES:
            raise ModelOutputError("compact body exceeds size limit")
        source_ids = memory["source_memory_ids"]
        if (not isinstance(source_ids, list) or len(source_ids) != 1
                or not isinstance(source_ids[0], str) or not source_ids[0]):
            raise ModelOutputError("compact replacement requires one source memory ID")
        ...
        consumed.add(key)
        normalized.append({"source_memory_ids": [source_id], "body": body})
...
def parse_compact_output(
    ...
    if not isinstance(raw, str) or len(raw.encode("utf-8")) > COMPACT_MAX_BYTES:
        raise ModelOutputError("compact response exceeds size limit")
